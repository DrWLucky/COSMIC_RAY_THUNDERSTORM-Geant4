...
public:
    void
    GeneratePrimaries(G4Event *) override;
    ...
    G4double
    Sample_one_RREA_gammaray_energy(const G4double &MinEner, const G4double &MaxEner, const G4double &E_cut);

    G4double
    Sample_one_PL_energy(const G4double &MinEner, const G4double &MaxEner, const G4double &index);

private:
    G4ParticleGun *fParticleGun; // pointer a to G4 service class
    G4ThreeVector
    isotropic_direction_rand_sample();

    G4ParticleTable *particleTable = G4ParticleTable::GetParticleTable();

    G4ThreeVector
    sample_gaussian_beam_direction(G4ThreeVector position, G4double OpeningAngle);

    G4ThreeVector
    sample_isotropic_beam_direction(G4ThreeVector position, G4double Opening_Ang);

    G4double
    sample_CR_particle_energy(G4int particleType);

    G4double
    interpolate(const std::vector<G4double> &xData, const std::vector<G4double> &yData, G4double x, bool extrapolate);

    G4ThreeVector
    sample_CR_secondary_position(double altitude);

    G4double
    sum_vector(std::vector<G4double> vec);
    ...
    std::vector<double>
    logspace(double min, double max, int nb);

    G4ThreeVector
    CR_direction_rand_sample(double cos_Sampled);

    parameter_set
    sample_parameter_set_from_PARMA_distrutions_cumu();
    ...
    void
    generator_output_for_test_momentum(const G4ThreeVector &mom);

...
// Air layers
void
DetectorConstruction::build_air_layers()
{
    sens_det_List.clear();
    ...
    for (unsigned int idx_alt = 0; idx_alt < altitudes.size() - 1; ++idx_alt)
    {
        ...
        solid_boxes.push_back(new G4Box("s_layer_" + std::to_string(idx_alt),
                                        World_XZ_size,
                                        delta_alt * G4double(0.50000000),
                                        World_XZ_size));

        logic_boxes.push_back(new G4LogicalVolume(solid_boxes.back(),
                                                  Airs[idx_alt],
                                                  "l_layer_" + std::to_string(idx_alt)));
        ...
        for (double &r_alt : Settings::RECORD_ALTITUDES) // loop over all requested record altitudes
        {
            ...
            if (ymin == alt_record)
            {
                if (Settings::USE_MAX_STEP_FOR_RECORD)
                    logic_boxes.back()->SetUserLimits(stepLimit_record); // maybe be overridden if this volume has also an electric field
        ...
        G4ThreeVector position_box{0,
                                   position_Y,
                                   0};

        phys_boxes.push_back(new G4PVPlacement(rotation_null,
                                               position_box,
                                               logic_boxes.back(),
                                               "atmosLayer_" + std::to_string(idx_alt) + "_"
                                                   + std::to_string(mean_alt / km),
                                               lWorld,
                                               false,
                                               0,
                                               false));
...
void
DetectorConstruction::create_thunderstorm_electric_fields()
{
    ...
    G4ElectroMagneticField *myEfield_local = new G4UniformElectricField_timeCut(G4ThreeVector(
        -EFIELD_VAL * std::sin(Settings::TILT), -EFIELD_VAL * std::cos(Settings::TILT), 0.));
...
std::vector<G4double>
DetectorConstruction::calculate_altitudes_list(G4double alt_min, G4double alt_max_construction, G4int nb_altitudes_)
// fills the vector altitudes
{
    ...
    for (G4int idx_alt = 0; idx_alt < nb_altitudes_; idx_alt++)   // geocentric altitudes
    {
        ...
        double alt = std::exp(std::log(alt_min)
                                  + (std::log(alt_max_construction) - std::log(alt_min)) * G4double(idx_alt)
                                      / G4double(nb_altitudes_ - 1));
    ...
    G4double alt_Min_Efield =
        (Settings::EFIELD_REGION_ALT_CENTER - Settings::EFIELD_REGION_LEN / G4double(2.00000000)) * km;
    G4double alt_Max_Efield =
        (Settings::EFIELD_REGION_ALT_CENTER + Settings::EFIELD_REGION_LEN / G4double(2.00000000)) * km;
...
bool
DetectorConstruction::hasDuplicates(const std::vector<G4double> &arr)
{
...
DetectorConstruction::Construct_Atmos_layers_Materials_simple(const vector<G4double> &altitudes_)
{
    std::vector<G4Material *> Airs;
    ...
    for (uint idx_alt = 0; idx_alt < altitudes_.size() - 1; idx_alt++)
    {
        ...
        else
        {
            ...
            gtd7_(input_iyd,
                  ...
                  input_mass,
                  output_D,
                  output_T); // MSIS, fortran function call
...
G4double
DetectorConstruction::interpolate(const vector<G4double> &xData,
...
G4double
DetectorConstruction::contains(const vector<G4double> &v, const G4double &x)
{
...
G4double
DetectorConstruction::get_scale(const G4double alt)
{
    // returns the atmospheric relative scale compared to sea level (>1)
    std::vector<G4double> alt_list = {0,
                                      0.500000000000000,
                                      1.000000000000000,
                                      1.500000000000000,
                                      2.000000000000000,
    ...
    std::vector<G4double> scale_list = {1.000000000000000,
                                        1.059301380991064,
                                        1.121238177128117,
                                        1.184377838328792,
                                        1.249042145593870,
